...
KANBAN_GUIDANCE = (
    ...
    "```\n"
    "\n"
    "### PR and commit hygiene (MANDATORY)\n"
    "\n"
    "1. **PR body MUST link the resolved issue.** Include `Closes #<issue-number>` "
    "in the PR body so GitHub auto-closes the issue when the PR merges. If the PR "
    "does not resolve a specific issue, explain why in the PR body.\n"
    "\n"
    "2. **Never use `git add -A` or `git add .`.** Only stage the files you actually "
    "created or modified. Run `git diff --stat` and `git diff --stat --cached` to "
    "verify before committing. This prevents accidentally committing:\n"
    "   - `.aider.chat.history.md` and `.aider.input.history` (should be in .gitignore "
    "because they are personal, never shared)\n"
    "   - `.worktrees/` directories (old worktree artifacts)\n"
    "   - `__pycache__/`, `.env` files, IDE config, or any other untracked files\n"
    "\n"
    "3. **Branch naming:** `hermes/issue-<NUMBER>-<short-slug>-<YYYYMMDD>`\n"
    "\n"
    "4. **PR body template:**\n"
    "```text\n"
    "## Summary\n"
