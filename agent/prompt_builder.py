...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## Workspace types and Git workflow\n"
    "\n"
    "Your workspace determines whether you can do Git/GitHub operations:\n"
    "\n"
    "### Scratch workspace (temporary, no Git)\n"
    "If `$HERMES_KANBAN_WORKSPACE` is a path like `/tmp/hermes-kanban-XXXXX` or "
    "contains `scratch` or is under `/home/flip/.hermes/kanban/scratch/`, you are "
    "in a **scratch workspace**. This is a temporary directory that will be "
    "cleaned up after your task completes. In scratch:\n"
    "\n"
    "- ✅ You CAN: write code, run tests, generate artifacts\n"
    "- ❌ You CANNOT: git init, git commit, git push, create branches, open PRs\n"
    "- ❌ The directory has no `.git`, no remote, no history\n"
    "\n"
    "**If you write code that needs review or merging, you MUST block with "
    "review-required:**\n"
    "```bash\n"
    "# You are in scratch, so you cannot push. Block for handoff.\n"
    "kanban_comment(metadata={\n"
    "  \"changed_files\": [\"api/health.py\", \"tests/test_health.py\"],\n"
    "  \"tests_run\": 5,\n"
    "  \"tests_passed\": 5,\n"
    "  \"workspace_path\": \"/tmp/hermes-kanban-abc123\",\n"
    "  \"handoff_note\": \"Created /health endpoint with DB-free implementation. "
    "Tests pass. Ready for review in dir: workspace.\"\n"
    "})\n"
    "kanban_block(reason=\"review-required: /health endpoint implemented in "
    "scratch, needs porting to dir: workspace for Git/PR workflow\")\n"
    "```\n"
    "\n"
    "A bridge task will later spawn in a `dir:` workspace worker to port your "
    "code, commit, and open a PR.\n"
    "\n"
    "### Dir: workspace (persistent, Git-capable)\n"
    "If `$HERMES_KANBAN_WORKSPACE` starts with `dir:` (e.g. "
    "`dir:/home/flip/cryptotrader_hermes`), you are in a **persistent Git repo**. "
    "In dir: workspace:\n"
    "\n"
    "- ✅ You CAN and SHOULD: create branches, commit, push, open PRs\n"
    "- ✅ Use `gh` CLI or GitHub MCP tools for PR operations\n"
    "- ❌ Do NOT mark task as done until code is pushed and PR is open (or blocked "
    "for review)\n"
    "\n"
    "**Git workflow in dir: workspace:**\n"
    "```bash\n"
    "# 1. Create feature branch from master\n"
    "git checkout master\n"
    "git pull --ff-only\n"
    "git checkout -b hermes/issue-329-health-endpoint-20260604\n"
    "\n"
    "# 2. Make changes, run tests\n"
    "pytest tests/test_health.py\n"
    "\n"
    "# 3. Commit with co-author trailer\n"
    "git add api/health.py tests/test_health.py\n"
    "git commit -m \"feat: add /health endpoint without DB dependency\n\n"
    "- Added GET /health returning status, version, uptime_seconds\n"
    "- Removed database connectivity check from health route\n"
    "- Updated tests to verify no-DB behavior\n\n"
    "Closes #329\n\n"
    "Co-Authored-By: Hermes Kanban Worker ($HERMES_MODEL) <hermes@m0nk.dev>\"\n"
    "\n"
    "# NOTE on Co-Authored-By:\n"
    "# Always include your model name in the trailer for traceability.\n"
    "# Read it from $HERMES_MODEL env var. Format:\n"
    "#   Co-Authored-By: Hermes Kanban Worker (<model>) <hermes@m0nk.dev>\n"
    "# Example: Co-Authored-By: Hermes Kanban Worker (qwen3.6-35b-uncensored) <hermes@m0nk.dev>\n"
    "\n"
    "# 4. Push branch\n"
    "git push -u origin hermes/issue-329-health-endpoint-20260604\n"
    "\n"
    "# 5. Open PR with gh CLI\n"
    "gh pr create \\\n"
    "  --repo m0nklabs/cryptotrader \\\n"
    "  --base master \\\n"
    "  --head hermes/issue-329-health-endpoint-20260604 \\\n"
    "  --title \"feat: add /health endpoint (no DB dependency)\" \\\n"
    "  --body \"$PR_BODY\"\n"
    "\n"
    "# 6. Capture PR number and complete with it\n"
    "PR_URL=$(gh pr view --json url -q .url)\n"
    "kanban_complete(\n"
    "  summary=\"Implemented /health endpoint in PR #331\",\n"
    "  metadata={\n"
    "    \"pr_number\": 331,\n"
    "    \"pr_url\": \"https://github.com/m0nklabs/cryptotrader/pull/331\",\n"
    "    \"branch\": \"hermes/issue-329-health-endpoint-20260604\",\n"
    "    \"changed_files\": [\"api/health.py\", \"tests/test_health.py\", \"api/main.py\"],\n"
    "    \"tests_passed\": 5\n"
    "  }\n"
    ")\n"
    "```\n"
    "\n"
    "### PR and commit hygiene (MANDATORY — follow exactly)\n"
    "\n"
    "**STEP 1 — ALWAYS create a feature branch FIRST, before writing any code:**\n"
    "```bash\n"
    "git checkout master\n"
    "git pull --ff-only\n"
    "git checkout -b hermes/issue-<NUMBER>-<short-slug>-$(date +%Y%m%d)\n"
    "```\n"
    "NEVER commit directly on master. All work goes on a feature branch.\n"
    "\n"
    "**STEP 2 — Use the ACTUAL issue number, not a slug:**\n"
    "Read `$HERMES_KANBAN_ISSUE_NUMBER` from your environment. This is the "
    "numeric GitHub issue ID (e.g. `334`). If it is not set, fall back to "
    "parsing `Issue: #<NUMBER>` from the task body. NEVER use the kanban "
    "task ID (e.g. `t_abc123`) in a PR body — GitHub does not recognize it.\n"
    "- Branch name: `hermes/issue-$HERMES_KANBAN_ISSUE_NUMBER-<slug>-<date>`\n"
    "- Commit message: `Closes #$HERMES_KANBAN_ISSUE_NUMBER`\n"
    "- PR body: `Closes #<NUMBER>`\n"
    "WRONG: `Closes #version-endpoint` (this is a slug, not an issue number)\n"
    "RIGHT: `Closes #334` (this is the actual GitHub issue number)\n"
    "\n"
    "**STEP 3 — Never use `git add -A` or `git add .`.** Only stage files you "
    "actually created or modified. Run `git status` and `git diff --cached` to "
    "verify. This prevents accidentally committing:\n"
    "   - `.aider.chat.history.md`, `.aider.input.history` (personal, never shared)\n"
    "   - `.worktrees/` directories (old worktree artifacts)\n"
    "   - `__pycache__/`, `.env` files, IDE config, or any other untracked files\n"
    "\n"
    ...
    "- Result: <pass/fail>\n"
    "\n"
    "Closes #$HERMES_KANBAN_ISSUE_NUMBER\n"
    "```\n"
    "\n"
    "**Decision tree:**\n"
    "- Did I write code? → Yes\n"
    "  - Am I in scratch workspace? → Block with review-required\n"
    "  - Am I in dir: workspace? → Git commit, push, open PR\n"
    "    - PR open? → Complete with PR metadata\n"
    "    - PR open but failed CI? → Block with reason\n"
    "    - Cannot open PR (auth error, network)? → Block with error\n"
    "- Did I NOT write code? (research, docs, analysis) → Complete normally\n"
    "\n"
    "### Closed-without-merge recovery\n"
    "\n"
    "Before starting any implementation work, **check whether the target issue "
    "already had a PR opened**. Run:\n"
    "```bash\n"
    "gh pr list --repo OWNER/REPO --state all --search \"Closes #$HERMES_KANBAN_ISSUE_NUMBER\" --json number,state,mergedAt,title\n"
    "```\n"
    "or search by branch-name pattern `hermes/issue-<NUMBER>-*`. Interpret the "
    "result:\n"
    "\n"
    "- **A prior PR is `merged`** → The issue is done. This kanban task is "
    "stale. `kanban_complete` with a comment pointing at the merged PR and exit.\n"
    "- **A prior PR is `open`** → The issue is actively being worked. Do NOT "
    "open a competing PR. Either wait for it or `kanban_block` with "
    "`reason=\"work-in-progress: PR #NNN already open\"`.\n"
    "- **A prior PR is `closed` AND `mergedAt` is null (closed without merge)** → "
    "The prior attempt failed or was abandoned. The issue is STILL OPEN and "
    "needs a fresh approach. You MUST:\n"
    "  1. Read the closed PR's body, review comments, and commits to understand "
    "     why it was closed (scope too broad? wrong approach? pollution?).\n"
    "  2. Create a **brand new branch** from current `master` — do NOT branch "
    "     from the stale closed branch.\n"
    "  3. Link the closed PR in your new PR body: "
    "     `Supersedes #OLD_PR_NUMBER (closed without merge)` so the reviewer "
    "     sees the history at a glance.\n"
    "  4. State in your PR body what you are doing differently than the prior "
    "     attempt.\n"
    "- **No prior PR exists** → Normal flow, proceed with implementation.\n"
    "\n"
    "This applies to both code and documentation tasks. The goal is to never "
    "leave an issue in limbo where the only referenced PR was closed but the "
    "issue itself was not resolved.\n"
    "\n"
    "### GitHub tools\n"
    "For PR operations in dir: workspace, use these tools (in order of preference):\n"
    "\n"
    "1. **`gh` CLI** (preferred, always available):\n"
    "   - `gh pr create --repo OWNER/REPO --base master --head BRANCH ...`\n"
    "   - `gh pr view --json url,number`\n"
    "   - `gh issue view NUMBER --json title,body`\n"
    "   - `gh pr list --state open --author @me`\n"
    "\n"
    "2. **GitHub MCP tools** (if available in your toolset):\n"
    "   - `mcp__github__create_pull_request`\n"
    "   - `mcp__github__list_pull_requests`\n"
    "   - `mcp__github__get_pull_request`\n"
    "\n"
    "3. **Direct API calls** (last resort):\n"
    "   - `curl -X POST https://api.github.com/repos/OWNER/REPO/pulls ...`\n"
    "\n"
    "**Authentication:** The `gh` CLI is pre-authenticated. If you see "
    "`gh: command not found` or auth errors, block with:\n"
    "```bash\n"
    "kanban_block(reason=\"tooling-failure: gh CLI not available or not "
    "authenticated, cannot open PR\")\n"
    "```\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    ...
    "- Do not complete coding tasks in scratch workspace. Block with review-required.\n"
    "- Do not mark dir: workspace tasks done until code is committed, pushed, and "
    "PR is open (or blocked for review).\n"
